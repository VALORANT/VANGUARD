...
export async function actualMassKickCmd(
  ...
  if (userIds.length > 100) {
    pluginData.state.common.sendErrorMessage(context, `Can only masskick max 100 users at once`);
    return;
  }
  ...
  for (const userId of userIds) {
    ...
    if (member && !canActOn(pluginData, author, member)) {
      pluginData.state.common.sendErrorMessage(context, "Cannot masskick one or more users: insufficient permissions");
      return;
    }
  }
  ...
  pluginData.state.masskickQueue.add(async () => {
    ...
    if (successfulKickCount === 0) {
      ...
      pluginData.state.common.sendErrorMessage(context, "All kicks failed. Make sure the IDs are valid.");
    } else {
