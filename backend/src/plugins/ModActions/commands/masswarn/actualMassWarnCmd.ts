...
export async function actualMassWarnCmd(
  ...
  if (userIds.length > 100) {
    pluginData.state.common.sendErrorMessage(context, `Can only masswarn max 100 users at once`);
    return;
  }
  ...
  for (const userId of userIds) {
    ...
    if (member && !canActOn(pluginData, author, member)) {
      pluginData.state.common.sendErrorMessage(context, "Cannot masswarn one or more users: insufficient permissions");
      return;
    }
  }
  ...
  pluginData.state.masswarnQueue.add(async () => {
    ...
    if (successfulWarnCount === 0) {
      ...
      pluginData.state.common.sendErrorMessage(context, "All warns failed. Make sure the IDs are valid.");
    } else {
